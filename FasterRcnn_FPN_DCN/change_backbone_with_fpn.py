...
def create_model(num_classes):
    ...
    # print(backbone)
    return_layers = {"features.6": "0",   # stride 8 下采样8倍
                     "features.12": "1",  # stride 16 下采样16倍
                     "features.16": "2"}  # stride 32 下采样32倍
    # 提供给fpn的每个特征层channel
    ...
    backbone_with_fpn = BackboneWithFPN(new_backbone,
                                        ...
                                        out_channels=256,  # FPN将通道统一设置成多少如这里是256
                                        extra_blocks=LastLevelMaxPool(),  # FPN右上角的maxpool层
                                        re_getter=False)  # 在官方的基础上新增的可以不重构模型，因为上面重构过了
    '官方给的重构方法只能重构块，如layer1等，这里可以将块下的层加入进去'
    '改一下'
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # save weights
        if epoch in range(args.start_epoch, args.epochs, 5):
            save_files = {
